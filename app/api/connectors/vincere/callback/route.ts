...
export async function GET(request: NextRequest) {
  ...
  try {
    // BYO-OAuth: exchange against the workspace's own app, saved as a 'pending'
    // connection by saveOAuthAppAction before this redirect.
    ...
    const clientSecret = connection?.oauth_client_secret_cipher
      ...
      : env.vincereClientSecret || undefined;
    if (!clientId) {
      return clearCookie(
      ...
      .upsert(
        {
          ...
          // Preserve the workspace's OAuth app credentials (needed for refresh).
    ...
    return clearCookie(
      NextResponse.redirect(settingsUrl(origin, "connected=vincere")),
    );
  } catch (err) {
