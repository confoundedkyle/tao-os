// Country reference data for the country picker and phone-code dropdown.
// Plain data module — safe to import from both server and client code.
...
// Alphabetical by name. Dial codes are the country calling codes; several
// countries legitimately share one (e.g. +1 across NANP, +7 RU/KZ).
...
// Phone-code order: by dial code, padded with trailing zeros so that, e.g.,
// +421 sorts before +50 ("4210" < "5000"). Ties broken by country name.
export const PHONE_CODES: Country[] = [...COUNTRIES].sort((a, b) => {
  const ka = a.dial.padEnd(MAX_DIAL_LEN, "0");
  const kb = b.dial.padEnd(MAX_DIAL_LEN, "0");
  if (ka !== kb) return ka < kb ? -1 : 1;
  return a.name.localeCompare(b.name);
});

/** Split a stored phone string ("+421 905…") into its dial code and number. */
export function splitPhone(value: string | null | undefined): {
  dial: string;
  number: string;
} {
  const v = (value ?? "").trim();
  if (!v.startsWith("+")) return { dial: "", number: v };
  const digits = v.slice(1).replace(/\D/g, "");
  // Longest matching dial-code prefix wins (so +1809 beats +1).
  const dials = [...new Set(COUNTRIES.map((c) => c.dial))].sort(
    (a, b) => b.length - a.length,
  );
  for (const d of dials) {
    if (digits.startsWith(d)) {
      const rest = v.slice(1 + d.length).replace(/^[\s-]*/, "");
      return { dial: d, number: rest };
    }
  }
  return { dial: "", number: v };
}

/** Combine a dial code and local number into a single stored phone string. */
export function joinPhone(dial: string, number: string): string {
  const n = number.trim();
  if (!dial) return n;
  if (!n) return ""; // a dial code with no number isn't a phone number
  return `+${dial} ${n}`;
}
